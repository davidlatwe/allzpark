...
class Environment(AbstractDockWidget):
    ...
    def __init__(self, ctrl, parent=None):
        ...
        layout = QtWidgets.QVBoxLayout(pages["native"])
        layout.setContentsMargins(0, 0, 0, 0)
        layout.addWidget(widgets["native"])

        widgets["view"].setSortingEnabled(True)
        widgets["native"].setSortingEnabled(True)
        ...
        panels["central"].addTab(pages["native"], "Base")
    ...
    def set_model(self, model_):
        ...
        self._widgets["native"].setModel(proxy_model)
        parent_env_model.load(self._ctrl.state["nativeEnviron"].copy())
